...
export default function WordsBunch() {
  ...
  return (
    <div className={styles.wordBlock}>
      ...
      {current > 2 ? <NavLink to={(current - 2).toString() || ''}><li>{current - 2}</li></NavLink> : null}
      {current > 1 ? <NavLink to={(current - 1).toString() || ''}><li>{current - 1}</li></NavLink> : null}
      <NavLink to={(+page!).toString() || ''}><li>{current}</li></NavLink>
      {current < 29 ? <NavLink to={(current + 1).toString() || ''}><li>{current + 1}</li></NavLink> : null}
      {current < 28 ? <NavLink to={(current + 2).toString() || ''}><li>{current + 2}</li></NavLink> : null}
